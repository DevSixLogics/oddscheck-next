...
export async function generateMetadata({ params }) {
  ...
  // CMS /seo-settings (race_detail.info) drives this; with no node it falls back to
  // the race name from the feed — no static copy.
  const cms = await cmsSeo({
    ...
    canonicalPath: `/race/${id}`,
  });
  return cms || { title: race.nm || "Racecard", description: null, alternates: { canonical: `/race/${id}` } };
}
